...
let routes = (app) => {
    ...
    app.post('/category', upload.single('image'), async (req, res) => {
        ...
        if (imageLink) {
            try {
                let category = new Category({ ...req.body, type: parseInt(req.body.type), image: imageLink, createdBy: responses.data.id });
                await category.save()
                res.json(category)
            }
            catch (err) {
    ...
    app.get('/category', async (req, res) => {
        ...
        const type = parseInt(req.query.type) || false;

        try {
            if (type) {
                const count = await Category.count({ type: type })
                let category = await Category.find({ type: type }).sort({ createdAt: -1 }).limit(limit).skip(page).populate("createdBy", "firstname lastname role")
                res.json({ category, pageNumber: Math.round((count / 16) + 0.4) })
            }
            else {
